...
class ReadMeRoutesChartMapMixin:
    @staticmethod
    ...
    def __add_geometry__(geometry, color):
        ax = plt.gca()
        ReadMeRoutesChartMapMixin.__get_gdf__(geometry).plot(
    ...
    def __draw_paths__(self):
        ...
        for i_route, route in enumerate(
            ...
            start_point_gdf = ReadMeRoutesChartMapMixin.__get_gdf__(
                [Point(start)]
            )
            end_point_gdf = ReadMeRoutesChartMapMixin.__get_gdf__(
                [Point(end)]
            )
            x_start, y_start = (
    ...
    def __draw_points__(self, location_list, name_to_lnglat):
        ...
        for i_location, location in enumerate(location_list, start=1):
            ...
            point_gdf = ReadMeRoutesChartMapMixin.__get_gdf__([Point(lnglat)])
